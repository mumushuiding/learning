...
结构化程序设计方法主要体现在过程的 ***功能划分*** 与过程内部的 ***编写规则*** 上，因此它是一种规范的过程化程序设计思想。

根本是要解决 ***算法与数据*** 的关系问题。

将程序中涉及的数据进行分类，独立地使用数据结构云描述其数据和处理
...
***抽象*** 数据类型：描述数据的 ***组织*** 和相关的 ***操作*** ，反映了问题的抽象模型。

### 1.7.2 面向对象的程序设计 （Object-Oriented Programming）

通过自建层次式的类型，使得 ***问题*** 中的逻辑结构反映为 ***实体*** 间的层次关系。

基于对象的程序设计本质上还是过程化的，只是 ***数据关系*** 被提取出来成为一个 ***类***。
